...
import styles from "./SellContractScreen.styles";

export default function SellContractScreen() {
  const navigation = useNavigation();
  const route = useRoute();
  const { sellRequestId, contractId } = route.params || {};
  ...
  useEffect(() => {
    console.log("SellContractScreen params nhận được:");
    console.log("→ sellRequestId:", sellRequestId || "KHÔNG CÓ");
    console.log("→ contractId:", contractId || "KHÔNG CÓ");
  }, [sellRequestId, contractId]);

  /* ================= LOAD CONTRACT ================= */
  useEffect(() => {
    if (contractId) {
      loadContractById(contractId);
    } else if (sellRequestId) {
      console.warn("Không có contractId");
      Alert.alert(
        "Thông báo",
  ...
  /* ================= UI ================= */
  if (!sellRequestId && !contractId) {
    return (
      <View style={styles.container}>
        <Text style={styles.errorText}>
          Không tìm thấy thông tin yêu cầu bán hoặc hợp đồng.
        </Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Hợp đồng bán cổ phần</Text>
      </View>
